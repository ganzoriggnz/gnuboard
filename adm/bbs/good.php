...
if(isset($_POST['js']) && $_POST['js'] === "on") {
    ...
    // hulan tailbar bolgoson gishuun bish ch like darah bolomjtoi
    if (!$is_member)
    {
    ...
    if ($good == 'good' || $good == 'nogood')
    {
        ...
        //     $row = sql_fetch($sql);
        // }
        //  ene hurtel nemsen 회원일 경우 회원 아이디로 중복투표를 방지하고 비회원일 경우 기록된 아이피를 확인하여 중복투표를 방지합니다.
        ...
        else
        {
            ...
            // 내역 생성
            // if (!$is_member) {
            // sql_query(" insert {$g5['board_good_table']} set bo_table = '{$bo_table}', wr_id = '{$wr_id}', mb_id = '{$_SERVER['REMOTE_ADDR']}', bg_flag = '{$good}', bg_datetime = '".G5_TIME_YMDHIS."' ");
            // } else {
            // sql_query(" insert {$g5['board_good_table']} set bo_table = '{$bo_table}', wr_id = '{$wr_id}', mb_id = '{$member['mb_id']}', bg_flag = '{$good}', bg_datetime = '".G5_TIME_YMDHIS."' ");
            // }

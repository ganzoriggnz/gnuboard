...
function URI(uri) {
  ...
  this.toAbsolute = function (location) {
    ...
    if (
      URIAbs.scheme !== null &&
      URIAbs.scheme.toLowerCase() === baseUri.scheme.toLowerCase()
    ) {
      URIAbs.scheme = null;
    }
...
cheditor.prototype = {
  ...
  setFolderPath: function () {
    if (
      this.config.editorPath.charAt(this.config.editorPath.length - 1) !== "/"
    ) {
  ...
  resetDoc: function () {
    ...
    try {
      ...
      this.doc = GB.browser.msie
        ? this.editArea.document
        : this.cheditor.editArea.contentDocument;
      this.resetData();
      return true;
    } catch (e) {
      alert(e.toString());
  ...
  resetDocumentBody: function () {
    this.doc.body.parentNode.replaceChild(
  ...
  setDocumentProp: function () {
    var oSheet,
      bodyCss =
        ...
        "; font-family:" +
        this.config.editorFontName +
        "; color:" +
        this.config.editorFontColor +
        "; margin:" +
  ...
  handleBeforePaste: function () {
    ...
    if (startOffset < 1 && commonAncestorContainer.nodeType === GB.node.text) {
      commonAncestorContainer.parentNode.insertBefore(
        wrapper,
        commonAncestorContainer
      );
    } else {
  ...
  resizeStart: function (evt) {
    var self = this;
    self.currentRS.elNode =
      self.cheditor.mode === "code"
        ? self.cheditor.textContent
        : self.cheditor.editArea;
    self.currentRS.cursorStartY = self.resizeGetY(evt);
    self.currentRS.elStartTop = parseInt(
      self.currentRS.elNode.style.height,
      10
    );
  ...
  resizeMove: function (evt) {
    ...
    this.config.editorHeight = this.currentRS.elNode.style.height =
      height + "px";
    this.stopEvent(evt);
  },
  ...
  resizeStop: function (evt) {
    ...
    if (this.cheditor.mode === "code") {
      this.config.editorHeight =
        parseInt(this.config.editorHeight, 10) +
        parseInt(this.cheditor.textContent.getAttribute("xbar-height"), 10) +
        "px";
  ...
  initTemplate: function () {
    ...
    if (window.XMLHttpRequest) {
      ...
      showError(
        "현재 브라우저에서 " + self.templateFile + " 파일을 사용할 수 없습니다."
      );
    }
  },
  /*
...
*/
  ...
  toolbarMouseOverUp: function (elem) {
    ...
    this.setToolbarBgPosition(
      elem.button,
      "0 " + (~((elem.pos + 1) * elem.height) + 1) + "px"
    );

    if (
      (elem.name === "combobox" && elem.prev && elem.prev.checked) ||
  ...
  toolbarMouseDownOut: function (elem, mousedown) {
    if (elem.next && elem.next.checked && !mousedown) {
      this.setToolbarBgPosition(
        elem.button,
        ~(elem.width * 2) + 1 + "px " + (~(elem.pos * elem.height) + 1) + "px"
      );
    }
    if (elem.prev) {
      ...
      if (elem.prev.checked) {
        this.setToolbarBgPosition(
          elem.prev.button,
          "0 " + (~((elem.prev.pos + 2) * elem.prev.height) + 1) + "px"
        );
        return;
      }
      if (mousedown) {
        this.setToolbarBgPosition(
          ...
          ~(elem.prev.width * 2) +
            1 +
            "px " +
            (~(elem.prev.pos * elem.prev.height) + 1) +
            "px"
        );
      } else {
        this.setToolbarBgPosition(
  ...
  toolbarSetBackgroundImage: function (elem, disable) {
    var css = elem.firstChild.className,
      tbEnable =
        this.cheditor.toolbarGrayscale && elem.firstChild.style.backgroundImage;

    css = css.replace(/-disable$/i, "");
    if (disable) {
      if (tbEnable) {
        elem.firstChild.style.backgroundImage =
          "url(" + this.cheditor.toolbarGrayscale + ")";
      }
      css = css + "-disable";
  ...
  toolbarUpdate: function (srcElement) {
    ...
    if (
      !bTable &&
      sType === GB.selection.element &&
      (pNode.nodeName.toLowerCase() === "table" ||
        pNode.nodeName.toLowerCase() === "td" ||
      ...
      switch (cmd) {
        ...
        case "UnLink":
          if (GB.browser.firefox) {
            this.toolbarDisable(
              btn,
              pNode.nodeName.toLowerCase() !== "a" &&
                !pNode.getAttribute("href")
            );
          } else {
            this.toolbarDisable(
              btn,
              this.doc.queryCommandEnabled(cmd) === false
            );
          }
          break;
        case "FormatBlock":
        ...
        case "BackColor":
          ...
          if (fontAttr && !/^[rgb|#]/.test(fontAttr)) {
            fontAttr = (
              ((fontAttr & 0x0000ff) << 16) |
              (fontAttr & 0x00ff00) |
              ((fontAttr & 0xff0000) >>> 16)
            ).toString(16);
              ...
              if (GB.lineHeightBlockTags[el.nodeName.toLowerCase()]) {
                ...
                if (css) {
                  for (j = 0; j < css.length; j++) {
                    if (
                      css[j].name.toLowerCase() === "text-align" &&
                      css[j].value === alignment[cmd]
                    ) {
                      state = true;
                      break;
                    }
            ...
            if (btn.type === "combo" && btn.name === btn.next.node) {
              ...
              this.setToolbarBgPosition(
                ...
                ~btn.next.width +
                  1 +
                  "px " +
                  (~((btn.next.pos + 1) * btn.next.height) + 1) +
                  "px"
              );
            }
          } else {
    ...
    self.addEvent(elem, "mousedown", function (ev) {
      if (!obj.checked && !obj.disabled) {
        ...
        if (obj.prev && obj.prev.type === "combo" && !obj.prev.checked) {
          self.setToolbarBgPosition(
            obj.prev.button,
            "0 " +
              (~(
                (self.getToolbarBgPosition(obj.prev.button) + 1) *
                obj.prev.height
              ) +
                1) +
              "px"
    ...
    comboOut = function (combo, startPos) {
      self.setToolbarBgPosition(
        ...
        startPos +
          "px " +
          (~(
            (self.getToolbarBgPosition(combo.button) +
              (combo.checked ? 2 : 1)) *
            combo.height
          ) +
            1) +
  ...
  showToolbar: function (toolbar, toolbarWrapper) {
    ...
    self.toolbar.iconDisable =
      self.config.iconPath + tmpArr[0] + "-disable" + "." + tmpArr[1];
    ...
    for (i = 0; i < grpNum; i++) {
      ...
      for (j = 0; j < btnLen; j++) {
        ...
        if (
          ...
          self.config["use" + btnName] !== true
        ) {
          continue;
        }
        if (
          attr.getAttribute("type") === "combobox" &&
          self.config["use" + attr.getAttribute("node")] !== true
        ) {
          continue;
        }
        ...
        case "viewMode":
          ...
          if (child.hasChildNodes()) {
            ...
            for (i = 0; i < tab.length; i++) {
              ...
              if (
                (tabId === "code" && self.config.useSource === false) ||
...
*/
  ...
  loadTemplate: function (xmlDoc) {
    ...
    cdata = tmpl
      .getElementsByTagName("PopupWindow")
      .item(0)
      .getElementsByTagName("Html")
      .item(0);
    if (!cdata) {
    ...
    var self = this,
      showEditor = function () {
        ...
        if ((GB.browser.msie && GB.browser.version > 8) || GB.browser.a) {
          ...
          grayImage.style.width = "750px";
          grayImage.style.height = "16px";
        } else {
  ...
  fullScreenMode: function () {
      ...
      containerReSize = function () {
        ...
        if (self.cheditor.mode === "code") {
          ...
          height =
            ...
            6 -
            parseInt(self.cheditor.textContent.getAttribute("xbar-height"), 10);
          self.cheditor.textContent.style.height = height + "px";
        }
        self.cheditor.editArea.style.height =
          windowSize.height - except - 6 + "px";
      },
      onMouseDownEventHandler = function (evt) {
        self.resizeStart(evt);
      };
    ...
    if (!self.setFullScreenMode) {
      ...
      while (child) {
        if (
          child.className !== "cheditor-editarea-wrapper" &&
      ...
      if (GB.browser.msie && GB.browser.version < 7) {
        self.cheditor.fullScreenFlag.parentNode.replaceChild(
          container,
          self.cheditor.fullScreenFlag
        );
      }
  ...
  setPulldownClassName: function (labels, pNode) {
    ...
    for (; i < labels.length; i++) {
      ...
      if (
        ...
        pNode.firstChild.firstChild.firstChild.nodeValue
      ) {
        label.parentNode.style.backgroundImage =
          "url(" + this.config.editorPath + "icons/checked.png)";
        ...
        var cmd =
          ...
          list =
            cmd === "InsertUnOrderedList"
              ? GB.listStyle.unOrdered
              : GB.listStyle.ordered,
          i,
          div,
          label,
          li,
  ...
  showColorMenu: function (pNode) {
    var menu = pNode.getAttribute("name"),
      ...
      selectedColor = this.colorConvert(
        pNode.lastChild.style.backgroundColor,
        "hex"
      ),
      i,
      len,
    ...
    this.toolbar[menu].colorNode.selectedValue.style.backgroundColor =
      selectedColor;
    ...
    for (i = 0; i < len; i++) {
      ...
      if (node.id && node.id.toLowerCase() === selectedColor.toLowerCase()) {
        node.style.backgroundImage =
          'url("' + this.config.iconPath + '/color_picker_tick.png")';
        node.style.backgroundRepeat = "no-repeat";
        node.style.backgroundPosition = "center center";
      }
    }
    this.toolbar[menu].colorNode.selectedValue.style.backgroundImage =
      'url("' + this.config.iconPath + '/color_picker_tick.png")';
    this.toolbar[menu].colorNode.selectedValue.style.backgroundRepeat =
      "no-repeat";
    this.toolbar[menu].colorNode.selectedValue.style.backgroundPosition =
      "center center";
  ...
  showFontTypeMenu: function (pNode) {
    ...
    self.setPulldownClassName(
      elem.firstChild.getElementsByTagName("LABEL"),
      pNode
    );
    self.windowPos(pNode, menu);
    self.displayWindow(pNode, menu);
  ...
  toolbarButtonOver: function (elemButton) {
    var nTop = elemButton.style.top.substring(
      0,
      elemButton.style.top.length - 2
    );
              ...
              for (k = 0; k < links.length; ++k) {
                ...
                if (
                  linkRange.compareBoundaryPoints(range.END_TO_START, range) <
                    1 &&
                  linkRange.compareBoundaryPoints(range.START_TO_END, range) >
                    -1
                ) {
    ...
    if (!this.W3CRange) {
      ...
      try {
        ...
        while (pNode && pNode.nodeName.toLowerCase() !== "body") {
          if (
            pNode.nodeName.toLowerCase() === "p" ||
            pNode.nodeName.toLowerCase() === "div"
          ) {
        ...
        if (obj && obj.hasChildNodes()) {
          ...
          do {
            if (
              ...
              !this.undefined(child.name) &&
              !this.undefined(child.value)
            ) {
              params.push({
  ...
  insertHTML: function (html) {
    if (!this.W3CRange) {
      this.getRange().pasteHTML(
        this.toType(html) === "string" ? html : html.outerHTML
      );
    } else {
    ...
    (function findBogusSpacer(elem) {
      ...
      for (; i < elem.childNodes.length; i++) {
        ...
        if (
          ...
          (node.nodeType === GB.node.comment &&
            node.nodeValue === self.cheditor.bogusSpacerName)
        ) {
          result.push(node);
        }
        if (
          node.nodeType === GB.node.text &&
          node.nodeValue === "\u00a0" &&
          !node.nextSibling
        ) {
          result.push(node);
  ...
  ieGetRangeAt: function (range) {
    ...
    function convert(result, bStart) {
      ...
      while (
        cursor.compareEndPoints(compareStr, point) > 0 &&
  ...
  applyLineHeight: function (opt) {
    ...
    try {
      if (startContainer === endContainer) {
        ...
        while (startLeaf) {
          if (
            startLeaf.nodeName.toLowerCase() === "body" ||
            isBlockElement(startLeaf.nodeName)
          ) {
            break;
          }
        ...
        while (endLeaf) {
          if (
            endLeaf.nodeName.toLowerCase() === "body" ||
            isBlockElement(endLeaf.nodeName)
          ) {
            break;
          }
  ...
  doInsertImage: function (imgs, para, insertSpace) {
    ...
    function checkPara(pNode) {
      ...
      self.getNodeTree(pNode, function (node) {
        ...
        if (node.type === GB.node.text) {
          ...
          node.type === GB.node.element &&
          node.name === "br" &&
          node.node.className === self.cheditor.bogusSpacerName
        ) {
          result = false;
        }
      });
    ...
    if (lastNode) {
      if (para) {
        ...
        this.placeCaretAt(
          lastNode.nextSibling ? lastNode.nextSibling : lastNode,
          false
        );
      } else {
        if (!this.W3CRange) {
  ...
  removeEmptyBogusTag: function (content) {
    if (
      /^<(p|div)([^>]+?)?>(&nbsp;|\s+?)<\/(p|div)>$/g.test(
        this.trimSpace(content)
      )
    ) {
      return "";
    }
  ...
  richMode: function () {
    ...
    if (
      this.cheditor.mode === "code" &&
      typeof this.resizeTextContent !== "undefined"
    ) {
      this.cheditor.textContent.focus();
      ...
      if (textContent.charAt(startPos) === ">") {
        ...
        while (cursor > -1) {
          ...
          if (
            char === "&" &&
            textContent.charAt(cursor + 1) === "n" &&
            textContent.charAt(cursor + 2) === "b" &&
            textContent.charAt(cursor + 3) === "s" &&
            textContent.charAt(cursor + 4) === "p" &&
  ...
  editMode: function () {
    var self = this,
      ...
      startNodeValue = "startBogusNode",
      endNodeValue = "endBogusNode",
      startNodeCommentTag = "<!--" + startNodeValue + "-->",
      endNodeCommentTag = "<!--" + endNodeValue + "-->",
      range = this.restoreRange();
    ...
    if (!content) {
      ...
      startEndNode.startNode.parentNode.replaceChild(
        startNode,
        startEndNode.startNode
      );
      startEndNode.endNode.parentNode.replaceChild(
        endNode,
        startEndNode.endNode
      );
      ...
      content = content
        .replace(startNodeCommentTag, "")
        .replace(endNodeCommentTag, "");
    }
    ...
    resize =
      ...
      (editorHeight - textContent.offsetHeight) -
      scrollBarHeight -
      borderHeight;
    ...
    this.resizeTextContent = function () {
      ...
      resize =
        textContent.offsetWidth +
        (editorWidth - textContent.offsetWidth) -
        scrollBarWidth -
        borderWidth;
  ...
  previewMode: function () {
    var content,
      oSheet,
      pos = this.getTextContentSelectionPos(false);

    if (
      this.cheditor.mode === "code" &&
      typeof this.resizeTextContent !== "undefined"
    ) {
      this.cheditor.textContent.setAttribute(
        "start-pos",
  ...
  convertContentsSpacer: function (content) {
    var self = this,
      bogusBr = true,
      excepted =
        '<span id="startBogusNode"></span><span id="endBogusNode"></span>',
      reSpacer = new RegExp(
        "<([^>]+)>(" +
          excepted +
          ")?(?:&nbsp;|s+?|\u00a0)(" +
    ...
    content = content.replace(
      ...
      function (all, open, excepted_a, excepted_b, close) {
        var tagName = self.trimSpace(open.split(" ")[0]).toLowerCase(),
          rdata = null;
        if (GB.lineHeightBlockTags[tagName] || GB.textFormatTags[tagName]) {
          rdata = "<" + open + ">" + (excepted_a || "");
          rdata += bogusBr
            ? '<br class="' + self.cheditor.bogusSpacerName + '" />'
  ...
  putContents: function (content) {
    if (this.config.fullHTMLSource) {
      content = content.substr(content.search(/<html/gi) + 1);
  ...
  getElementStyle: function (elem) {
    return window.getComputedStyle
      ? this.doc.defaultView.getComputedStyle(elem, null)
      : elem.currentStyle;
  },
  ...
  getContents: function (startEndNode) {
    ...
    function checkChildNodes(child) {
      ...
      do {
        if (
          child !== startEndNode.startNode &&
          child !== startEndNode.endNode
        ) {
          break;
        }
        child = child.nextSibling;
      ...
      switch (node.type) {
        ...
        case GB.node.element:
          ...
          if (
            ...
            !checkChildNodes(node.node.firstChild)
          ) {
            node.node.appendChild(self.createNbspTextNode());
          }
          if (GB.newLineBeforeTags[node.name] && allowedIndent) {
            if (
              ...
              node.node.firstChild.nodeType === GB.node.element
            ) {
              node.node.insertBefore(
                insertTabSpace(node.indent + 1),
                node.node.firstChild
              );
    ...
    for (i in indentNodes) {
      if (indentNodes.hasOwnProperty(i)) {
        ...
        if (node.node.nextSibling) {
          node.node.nextSibling.parentNode.insertBefore(
            insertTabSpace(node.indent),
            node.node.nextSibling
          );
        } else {
    ...
    if (msie_c) {
      mydoc = mydoc
        ...
        .replace(/<(\/?)([A-Za-z]+)([^>]*)>/g, function (a, close, tag, attr) {
          attr = attr
            .replace(/\s(\w+)=([^'"\s>]+)/g, function (a, k, v) {
              return " " + k.toLowerCase() + '="' + v + '"';
            })
            .replace(/([A-Za-z\-]+)(?:\s*):\s+?/g, function (a, k) {
    ...
    mydoc = mydoc.replace(/<[^>]+>/gm, function (match) {
      ...
      if (self.config.colorToHex) {
        match = match.replace(
          /(background-color|color)\s*([:=])\s*(rgba?)\(\s*(\d+)\s*,\s*(\d+),\s*(\d+)\)/gi,
          function (all, p, s, rgb, r, g, b) {
            return (
              p +
              s +
              " " +
              self.colorConvert(rgb + "(" + r + "," + g + "," + b + ")", "hex")
            );
          }
        );
      } else {
        match = match.replace(
          /(background-color|color)\s*([:=])\s*(#[A-Fa-f0-9]{3,6})/gi,
  ...
  getBodyContents: function () {
    return this.cheditor.mode === "code"
  ...
  getBodyText: function () {
    ...
    return this.trimSpace(
      String(
        GB.browser.msie ? this.doc.body.innerText : this.doc.body.textContent
      )
    );
  },
  ...
  makeRandomString: function () {
    var chars =
        "_-$@!#0123456789ABCDEFGHIJKLMNOPQRSTUVWXTZabcdefghiklmnopqrstuvwxyz",
      len = 32,
      clen = chars.length,
      rData = "",
      i,
  ...
  windowPos: function (pNode, id) {
    var left = pNode.offsetLeft,
      box = this.pulldown[id];
  ...
  setColorTable: function (menu) {
    var self = this,
      ...
      onResetEventHandler = function () {
        colorPicker.fromString(
          self.colorConvert(selectedValue.style.backgroundColor, "hex")
        );
      },
    ...
    br.style.clear = "both";
    br.style.height = "0px";
    colorPicker = new GB.colorDropper(selected, {
  ...
  doOnKeyUp: function (evt) {
    var caretRange,
      ...
      clearBackgroundColor = function (cNode) {
        if (
          cNode.nodeType !== GB.node.element ||
          cNode.hasChildNodes() ||
          !cNode.getAttribute("style")
        ) {
          return cNode;
        }
  ...
  applyBogusClassName: function (range) {
    ...
    if (
      range.startOffset < 1 &&
      (!node.lastChild || node.lastChild.nodeName.toLowerCase() !== "br")
    ) {
      do {
  ...
  setWinPosition: function (oWin, popupAttr, windowSize) {
    ...
    oWin.style.left =
      Math.round(
        (this.cheditor.editArea.clientWidth - popupAttr.width) / 2 +
          windowSize.offsetLeft
      ) + "px";
  ...
  popupWinLoad: function (popupAttr) {
    var self = this,
      ...
      popWinResizeHeight = function (evt) {
        ...
        if (self.cheditor.popupElem.style.visibility !== "visible") {
          self.cheditor.popupElem.style.top =
            Math.ceil(
              parseInt(self.cheditor.popupElem.style.top, 10) -
                Math.ceil(self.cheditor.popupElem.clientHeight / 2)
            ) + "px";
      ...
      modalResize = function () {
        self.cheditor.modalBackground.style.height =
          windowSize.scrollHeight > windowSize.height
            ? windowSize.scrollHeight
            : windowSize.height + "px";

        if (window.scrollWidth > window.width) {
          self.cheditor.modalBackground.style.width =
            windowSize.width +
            (windowSize.scrollWidth - windowSize.width) +
            "px";
        } else {
    ...
    if (self.cheditor.popupFrameWrapper.hasChildNodes()) {
      self.cheditor.popupFrameWrapper.removeChild(
        self.cheditor.popupFrameWrapper.firstChild
      );
    }

    self.cheditor.popupTitle.appendChild(
      document.createTextNode(popupAttr.title)
    );
  ...
  popupWinClose: function () {
    ...
    if (this.cheditor.popupFrameWrapper.hasChildNodes()) {
      this.cheditor.popupFrameWrapper.removeChild(
        this.cheditor.popupFrameWrapper.firstChild
      );
    }
  ...
  createRange: function () {
    return this.W3CRange
      ? this.doc.createRange()
      : this.doc.body.createTextRange();
  },

  rangeCollapsed: function (range) {
    return this.W3CRange
      ? range.collapsed
      : !this.undefined(range.text) &&
          range.text.length === 0 &&
  ...
  getRangeElement: function (range) {
    return this.W3CRange
      ? this.getW3CRangeElement(range)
      : this.getIeRangeElement(range);
  },
  ...
  doCmd: function (cmd, opt) {
    var self = this,
      ...
      isTextVisible = function (elem) {
        return !(
          elem.firstChild.nodeType === GB.node.text &&
    ...
    if (cmd === "ClearTag") {
      if (
        confirm(
          "모든 HTML 태그를 삭제합니다. 계속하시겠습니까?\n(P, DIV, BR 태그와 텍스트는 삭제하지 않습니다.)"
        )
      ) {
        content = this.doc.body.innerHTML;
        this.doc.body.innerHTML = content.replace(
          /<(\/?)([^>]*)>/g,
          function (a, b, c) {
        ...
        if (GB.textFormatBlockTags[tagName]) {
          if (hr.nextSibling) {
            ...
            while (hr.nextSibling) {
              if (
                hr.nextSibling.nodeType === GB.node.text &&
                hr.nextSibling.nodeValue === ""
              ) {
                hr = hr.nextSibling;
                continue;
              }
              if (hr.nextSibling.parentNode !== pNode) {
                ...
                while (
                  node !== pNode &&
                  GB.textFormatTags[node.nodeName.toLowerCase()]
                ) {
          ...
          this.placeCaretAt(
            hRule.nextSibling,
            hRule.nextSibling.nodeType === GB.node.text
          );
        }
      } else {
        switch (cmd) {
          ...
          case "JustifyFull":
            ...
            pNode = this.getRangeElement(
              this.W3CRange ? this.getRange() : range
            );

            while (pNode && pNode.nodeName.toLowerCase() !== "body") {
              if (
                typeof pNode.getAttribute !== "undefined" &&
                pNode.getAttribute("align")
              ) {
                node = pNode;
                break;
              } else {
              ...
              if (found) {
                ...
                if (!GB.browser.msie) {
                  if (
                    ...
                    if (
                      pNode.lastChild &&
                      pNode.lastChild.nodeName.toLowerCase() === "br"
                    ) {
  ...
  cleanFromWord: function (tmpDoc) {
    ...
    doc = doc.replace(/<\/?v:[^>]*>/g, "");
    doc = doc.replace(/<\/?o:[^>]*>/g, "");
    doc = doc.replace(/<\/?st1:[^>]*>/g, ""); //doc = doc.replace(/lang=.?[^' >]*/ig, '');
    ...
    doc = doc.replace(/<p[^>]*/gi, "<p");
    doc = doc.replace(/windowtext/gi, "#000000");
    doc = doc.replace(/class=table/gi, "");
    doc = doc.replace(/<span[^>]*<\/span>/gi, "");
    return doc;
  },
  ...
  getCssValue: function (elem) {
    ...
    for (i = 0; i < len; i++) {
      q = css[i].split(":");
      style.push({
        name: this.trimSpace(q[0]).toLowerCase(),
        value: this.trimSpace(q[1]).toLowerCase()
      });
    }
    return style;
  },
  ...
  insertStartEndNode: function (range) {
    ...
    if (!this.W3CRange) {
      ...
      if (
        ...
        startNode.previousSibling.nodeType === GB.node.text &&
        startNode.previousSibling.nodeValue === ""
      ) {
        startNode.previousSibling.parentNode.removeChild(
          startNode.previousSibling
        );
      }
      if (
        endNode.nextSibling &&
        endNode.nextSibling.nodeType === GB.node.text &&
  ...
  doCmdPopup: function (cmd, opt, checked) {
    var self = this,
      ...
      headNodes = [],
      rootNode = null,
      zeroWidth = this.doc.createTextNode("\u200b"),
      inRange,
      ...
      searchTextNode = function (node, match) {
        ...
        for (; i < len; i++) {
          ...
          if (
            child.nodeType === GB.node.element &&
            child.hasChildNodes() === false &&
      ...
      clearPreviousLeaf = function (node) {
        ...
        while (
          leaf.lastChild &&
          leaf.nodeType === GB.node.element &&
          leaf !== startNode
        ) {
          rangeSelectNode(cursor, leaf);
          compare = compareBoundaryPoints(
            cursor,
            "StartToStart",
            startNodeRange
          );
          if (compare === 1) {
            ...
            if (
              !css &&
              (leaf.nodeName.toLowerCase() === "span" ||
                leaf.nodeName.toLowerCase() === "font")
            ) {
              while (leaf.firstChild) {
        ...
        if (
          leaf.nodeType === GB.node.text &&
          self.isTextVisible(leaf.nodeValue)
        ) {
      ...
      checkInRange = function (range, source) {
        return typeof range.inRange !== "undefined"
    ...
    try {
      if (cmd === "LineHeight") {
        ...
        if (cmd === "InsertOrderedList" || cmd === "InsertUnOrderedList") {
          ...
          pNode = this.W3CRange
            ? cursor.commonAncestorContainer
          ...
          while (pNode) {
            if (
              pNode.nodeName.toLowerCase() === "ol" ||
              pNode.nodeName.toLowerCase() === "ul"
            ) {
          ...
          if (!tempNodes.collapsed) {
            ...
            for (i = 0; i < len; i++) {
              ...
              if (
                ...
                tNode.previousSibling.nodeType === GB.node.text
              ) {
                if (
                  applyTextNodes[i + 1] &&
                  tNode.previousSibling === applyTextNodes[i + 1]
                ) {
                  applyTextNodes[i + 1].nodeValue += tNode.nodeValue;
                  tNode.parentNode.removeChild(tNode);
                } else {
                  tNode.nodeValue =
                    tNode.previousSibling.nodeValue + tNode.nodeValue;
                  tNode.previousSibling.parentNode.removeChild(
                    tNode.previousSibling
                  );
                  i--;
                }
              ...
              if (
                pNode.nodeName.toLowerCase() === "span" &&
                (pNode.firstChild === tNode ||
                  pNode.firstChild === startNode) &&
  ...
  modifyImage: function (img) {
    var self = this,
      ...
      getCssFloat = function (elem) {
        return typeof elem.style.styleFloat === "undefined"
          ? elem.style.cssFloat
      ...
      getCaptionNodes = function (img) {
        ...
        if (
          pNode.parentNode.nodeName.toLowerCase() === "div" &&
          pNode.parentNode.className === wrapperClassName
        ) {
    ...
    for (idx in imageWidthOpt) {
      if (imageWidthOpt.hasOwnProperty(idx)) {
        fmSelectWidth.options[fmSelectWidth.options.length] = new Option(
          imageWidthOpt[idx].desc,
          idx
        );
      }
    }
    ...
    wrapTextChecked = getCssFloat(
      caption && caption.wrapper ? caption.wrapper : img
    );

    for (idx in imageFloatOpt) {
      if (imageFloatOpt.hasOwnProperty(idx)) {
        wrapTextCheckBox = createInputForm(
          "checkbox",
          idx,
          "1",
          "wrap-checked"
        ...
        wrapTextIcon.src =
          this.config.iconPath + "image_align_" + idx + "_wt.png";
    ...
    for (idx in captionAlignOpt) {
      if (captionAlignOpt.hasOwnProperty(idx)) {
        fmSelectCaptionAlign.options[fmSelectCaptionAlign.options.length] =
          new Option(captionAlignOpt[idx], idx);
        if (
          caption &&
          caption.figCaption[0] &&
          caption.figCaption[0].style.textAlign
        ) {
          if (idx === caption.figCaption[0].style.textAlign) {
            fmSelectCaptionAlign.options[
              fmSelectCaptionAlign.options.length - 1
            ].selected = true;
          }
        }
      }
  ...
  modifyCell: function (ctd) {
    var self = this,
      ...
      getCellMatrix = function () {
        rows =
          ctb.rows && ctb.rows.length > 0
            ? ctb.rows
            : ctb.getElementsByTagName("tr");
      ...
      mergeCellDown = function () {
        ...
        if (rowIndex + ccrs < rows.length) {
          ...
          if (
            ...
            (crealIndex === 0 ||
              (crealIndex > 0 &&
                tm[rowIndex + ccrs][crealIndex - 1] !==
                  tm[rowIndex + ccrs][crealIndex]))
          ) {
            ncrs = rows[rowIndex + ccrs].cells[ncellIndex].rowSpan || 1;
            nccs = rows[rowIndex + ccrs].cells[ncellIndex].colSpan || 1;
            if (cccs === nccs) {
              html = self.trimSpace(
                rows[rowIndex + ccrs].cells[ncellIndex].innerHTML
              );
              html = html.replace(/^&nbsp;/, "");
              ctd.innerHTML += html;
        ...
        if (ctd.colSpan > 1) {
          ...
          for (i = 0; i < tm.length; i++) {
            if (i !== rowIndex && tm[i][realIndex] !== -1) {
              cs =
                rows[i].cells[tm[i][realIndex]].colSpan > 1
                  ? rows[i].cells[tm[i][realIndex]].colSpan
                  : 1;
      ...
      editSubmitEventHandler = function () {
        var width = self.trimSpace(
            document.getElementById("fm_cell_width").value
          ),
          height = self.trimSpace(
            document.getElementById("fm_cell_height").value
          ),
          bgcolor = self.trimSpace(
            document.getElementById("fm_cell_bgcolor").value
          );
        if (width) {
  ...
  doEditorEvent: function (evt) {
    ...
    if (
      !this.undefined(srcElement) &&
      srcElement.nodeType === GB.node.element
    ) {
      pNode = srcElement;
    } else {
      range = self.getRange();
    ...
    while (pNode && pNode.nodeType === GB.node.element) {
      ...
      if (
        pNode.tagName.toLowerCase() === "td" ||
        pNode.tagName.toLowerCase() === "th"
      ) {
        cmd = "cell";
        break;
      }
...
(function () {
  var dragWindow = {
    ...
    drag: function (e) {
      ...
      dragWindow.obj.root.onDragEnd(
        parseInt(
          dragWindow.obj.root.style[dragWindow.obj.hmode ? "left" : "right"],
          10
        ),
        parseInt(
          dragWindow.obj.root.style[dragWindow.obj.vmode ? "top" : "bottom"],
          10
        )
      );
  ...
  var colorDropper = {
    ...
    color: function (target, prop) {
      ...
      function removePicker() {
        ...
        colorDropper.picker.boxB.parentNode.removeChild(
          colorDropper.picker.boxB
        );
      }
      ...
      function drawPicker() {
        ...
        p.padM.style.width =
          self.pickerFace +
          2 * self.pickerInset +
          colorDropper.images.pad[0] +
          colorDropper.images.arrow[0] +
          "px";
        ...
        p.padM.style.backgroundImage =
          'url("' + self.iconDir + '/color_picker_cross.gif")';
        p.padM.style.backgroundRepeat = "no-repeat";
        p.sldM.style.backgroundImage =
          'url("' + self.iconDir + '/color_picker_arrow.gif")';
        p.sldM.style.backgroundRepeat = "no-repeat";
        p.pad.style.backgroundImage =
          'url("' + self.iconDir + "/" + padImg + '")';
      ...
      function getPickerDims(o) {
        return [
          2 * o.pickerInset +
            ...
            colorDropper.images.pad[0] +
            2 * o.pickerInset +
            2 * colorDropper.images.arrow[0] +
            colorDropper.images.sld[0],
      ...
      function redrawPad() {
        ...
        y = Math.round(
          (1 - self.hsv[yComponent]) * (colorDropper.images.pad[1] - 1)
        );
        colorDropper.picker.padM.style.backgroundPosition =
          self.pickerFace +
          self.pickerInset +
          x -
          Math.floor(colorDropper.images.cross[0] / 2) +
          "px " +
          (self.pickerFace +
            self.pickerInset +
            y -
            Math.floor(colorDropper.images.cross[1] / 2)) +
          "px";
        ...
        switch (modeID) {
          case 0:
            ...
            if (window.File && window.FileReader) {
              colorDropper.picker.sld.style.background =
                "linear-gradient(rgb(" +
      ...
      function redrawSld() {
        ...
        y = Math.round(
          (1 - self.hsv[yComponent]) * (colorDropper.images.sld[1] - 1)
        );
        colorDropper.picker.sldM.style.backgroundPosition =
          ...
          (self.pickerFace +
            self.pickerInset +
            y -
            Math.floor(colorDropper.images.arrow[1] / 2)) +
          "px";
      }
      ...
      function setPad(e) {
        ...
        switch (modeID) {
          case 0:
            self.fromHSV(
              x * (6 / (colorDropper.images.pad[0] - 1)),
              1 - y / (colorDropper.images.pad[1] - 1),
              null,
              leaveSld
            );
            break;
        ...
        switch (modeID) {
          case 0:
            self.fromHSV(
              null,
              null,
              1 - y / (colorDropper.images.sld[1] - 1),
              leavePad
            );
            break;
          case 1:

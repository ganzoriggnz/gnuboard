...
function showContents() {
  ...
  for (i = 0; i < uploadMaxNumber; i++) {
    if (i > 0 && i % 4 === 0) {
      document
        .getElementById('imageListWrapper')
        .appendChild(spacer(spacerNo++))
    }
  ...
  if (browser.msie && browser.ver < 7) {
    document.getElementById('imageListWrapper').style.padding =
      '5px 2px 5px 2px'
...
function getUploadedCount() {
  return document.getElementById('imageListWrapper').getElementsByTagName('img')
    .length
}
...
function init(dialog) {
  ...
  document
    ...
    .appendChild(document.createTextNode(uploadMaxNumber))

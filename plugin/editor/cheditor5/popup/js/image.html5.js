  ...
  for (; i < imgBox.length; i++) {
    if (
      (imgBox[i].className === 'imageBox' ||
        imgBox[i].className === 'imageBoxHighlighted') &&
      imgBox[i].id
    ) {
...
function reOrder() {
  ...
  for (i = 0; i < breakline.length; i++) {
    ...
    if (i + 1 === breakline.length) {
      ...
      imageListWrapper.insertBefore(
        spacer,
        document.getElementById(breakline[i])
      )
    }
  }
}
...
function showDelete() {
  ...
  btn.style.left =
    showThumbnailSize.width - parseInt(btn.style.width, 10) - 1 + 'px'
  btn.style.top = '-1px'
...
function startUpload(list) {
  ...
  for (i = 0; i < el.length; i++) {
    ...
    if (
      !imgBox.firstChild ||
      imgBox.firstChild.tagName.toLowerCase() !== 'img'
    ) {
...
function imgComplete(img, imgSize, boxId) {
  ...
  if (
    imgSize.width > showThumbnailSize.width ||
    imgSize.height > showThumbnailSize.height
  ) {
    if (imgSize.width > imgSize.height) {
      resizeW =
        imgSize.width > showThumbnailSize.width
          ? showThumbnailSize.width
          : imgSize.width
      resizeH = Math.round((imgSize.height * resizeW) / imgSize.width)
    } else {
      resizeH =
        imgSize.height > showThumbnailSize.height
          ? showThumbnailSize.height
          : imgSize.height
...
function removeImages() {
  ...
  for (i = 0; i < uploadMaxNumber; i++) {
    ...
    if (
      theImage.hasChildNodes() &&
      theImage.firstChild.tagName.toLowerCase() === 'img'
    ) {
...
function dragDropMove(e) {
  ...
  for (prop in divXPositions) {
    if (
      !divXPositions.hasOwnProperty(prop) ||
      divXPositions[prop].className === 'clear'
    ) {
      continue
    }

    if (
      divXPositions[prop] < leftPos &&
...
DoUpload.prototype = {
  ...
  imageResize: function (
    ...
    if (this.reader.watermark && addWaterMark) {
      ...
      ctx.drawImage(
        this.reader.watermark,
        canvas.width - this.reader.watermark.width,
        canvas.height - this.reader.watermark.height
      )
    }
  ...
  dataLoadHandler: function (evt) {
    ...
    if (evt.target.readyState === FileReader.DONE) {
      ...
      image.onload = function () {
        ...
        xhr.addEventListener(
          'loadend',
          function onLoadendImageHandler(xhrevt) {
  ...
  load: function () {
    ...
    if (file) {
      if (debug) {
        console.log(
          'File ' + this.index + ', Name: ' + file.name + ', Size: ' + file.size
        )
      }
      ...
      if (
        oEditor.config.imgWaterMarkUrl !== '' &&
        oEditor.config.imgWaterMarkUrl !== null
      ) {
        watermark = new Image()
...
function setResizeWidth() {
  ...
  if (value) {
    ...
    if (!isNaN(value) && value < oEditor.config.imgMaxWidth) {
      ...
      imageResizeInput.setAttribute(
        'placeholder',
        oEditor.config.imgMaxWidth.toString()
      )
    }
  }
}
...
function init(dialog) {
  ...
  if (imageResizeWidth > 0) {
    for (i = 0; i < oEditor.config.imgResizeValue.length; i++) {
      ...
      select.options[select.options.length] = new Option(
        ...
        value,
        false,
        value === oEditor.config.imgResizeSelected
      )
    }
  ...
  document
    .getElementById('maxImageNum')
    .appendChild(document.createTextNode(uploadMaxNumber.toString()))
  ...
  for (i = 0; i < elem.length; i++) {
    if (
      elem[i].name === 'alignment' &&
      elem[i].value === oEditor.config.imgDefaultAlign
    ) {

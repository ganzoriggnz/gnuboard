    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
        ...
        <tr>
            <td style="height:32px; border:0px solid">
                <form id="fattendance" name="fattendance" method="post" action="<?php echo G5_PLUGIN_URL ?>/attendance/m_attendance_write_update.php" enctype="multipart/form-data" style="margin:0px;">
                ...
                <script type="text/javascript">
                    ...
                    value[2] = "내가 있는 곳이 낙원이라. - 볼테르"
                    value[3] = "시간은 우리를 변화시키지 않는다. 시간은 단지 우리를 펼쳐 보일 뿐이다. - 막스 프리쉬"
                    value[4] = "큰 희망이 큰 사람을 만든다. - 토마스 풀러"
                    value[5] = "희망은 백일몽이다. - 아리스토텔레스"
                    value[6] = "사람은 실패가 아니라 성공하기 위해 태어난다. - 헨리 데이비드 소로우"
                    value[7] = "노력 없이 쓰인 글은 대개 감흥 없이 읽힌다. - 사무엘 존슨"
                    value[8] = "기회는 자기 소개서를 보내지 않는다. - 작자 미상"
                    value[9] = "시간은 너무 없고 할 일도 너무 없다. - 오스카 레반트"
                    value[10] = "가장 현명한 사람은 자신만의 방향을 따른다. - 에우리피데스"
                    value[11] = "경험을 현명하게 사용한다면, 어떤 일도 시간 낭비는 아니다. - 오귀스트 르네 로댕"
                    value[12] = "인간은 오직 사고(思考)의 산물일 뿐이다. 생각하는 대로 되는 법. - 마하트마 간디"
                    value[13] = "유머가 아예 없다면 인생을 불가능으로 바꾼다. - 꼴레뜨"
                    value[14] = "성공만큼 큰 실패는 없다. - 제럴드 내크먼"
                    value[15] = "성공한 사람이 될 수 있는데 왜 평범한 이에 머무르려 하는가? - 베르톨트 브레히트"
                    value[16] = "지성을 다하는 것이 곧 천도(天道)다(지성이면 감천이다). - 맹자"
                    value[17] = "인간의 감정은 누군가를 만날 때와 헤어질 때 가장 순수하며 가장 빛난다. - 장 폴 리히터"
                    value[18] = "잘 있거라! 우리가 언제 다시 만날지는 아무도 모른다. - 윌리엄 셰익스피어"
                    value[19] = "나만이 내 인생을 바꿀 수 있다. 아무도 날 대신해 해줄 수 없다. - 캐롤 버넷"
                    value[20] = "여가시간을 가지려면 시간을 잘 써라. - 벤자민 프랭클린"
                    value[21] = "희망만이 인생의 유일한 사랑이다. - 앙리 프레데릭 아미엘"
                    value[22] = "일 분 전만큼 먼 시간은 없다. - 짐 비숍"
                    value[23] = "미래를 결정짓고 싶다면 과거를 공부하라. - 공자"
                    value[24] = "덜 약속하고 더 해주어라. - 톰 피터스"
                    value[25] = "젊은이들에게 관대하라. - 유베날리스"
                    value[26] = "인생은 위험의 연속이다. - 다이앤 프롤로브"
                    value[27] = "희망은 어떤 상황에서도 필요하다. - 사무엘 존슨"
                    value[28] = "삶이 있는 한 희망은 있다. - 키케로"
                    value[29] = "우리의 목적은 성공이 아니라 봉사라야 한다. - 작자 미상"
                    value[30] = "운은 계획에서 비롯된다. - 브랜치 리키"
                    value[31] = "이별의 아픔 속에서만 사랑의 깊이를 알게 된다. - 조지 앨리엇"
                    value[32] = "긍정적 사고만큼 나를 우울하게 만드는 일은 없다. - 폴 퍼셀"
                    value[33] = "지금 적극적으로 실행되는 괜찮은 계획이 다음 주의 완벽한 계획보다 낫다. - 조지 S. 패튼"
                    value[34] = "티끌 모아 태산 / 천리 길도 한 걸음부터 - 공자"
                    value[35] = "웃음 없는 하루는 낭비한 하루다. - 찰리 채플린"
                    value[36] = "시간은 환상이다. 점심시간은 두 배로 그렇다. - 더글러스 애덤스"
                    value[37] = "운은 계획에서 비롯된다. - 브랜치 리키"
                    value[38] = "열정없이 사느니 차라리 죽는게 낫다. - 커트 코베인"
                    value[39] = "우리는 오늘은 이러고 있지만, 내일은 어떻게 될지 누가 알아요? - 윌리엄 셰익스피어"
                    value[40] = "그 어떤 것에서라도 내적인 도움과 위안을 찾을 수 있다면 그것을 잡아라. - 마하트마 간디"
                    value[41] = "그 여정이 바로 보상이다. - 스티브 잡스"
                    value[42] = "인생은 겸손에 대한 오랜 수업이다. - 제임스 M. 배리"
                    value[43] = "인생은 집을 향한 여행이다. - 허먼 멜빌"
                    value[44] = "시작이 반이다. - 아리스토텔레스"
                    value[45] = "성공의 8할은 일단 출석하는 것이다. - 우디 알렌"
                    value[46] = "성경은 게으름뱅이에게 빵을 약속하지 않는다. - 작자 미상"
                    value[47] = "실패하는 것은 곧 성공으로 한 발짝 더 나아가는 것이다. - 메리 케이 애쉬"
                    value[48] = "인생은 밀림 속의 동물원이다. - 피터 드 브리스"

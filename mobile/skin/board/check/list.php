...
if ($sca || $stx || $stx === '0') {     //검색이면
    ...
    // 검색 재작성 2020-07-06
    $sql_search .= " and left(wr_datetime, 10) = '{$date}'";
    ...
    */
    ...
    // 검색 재작성 2020-07-06
    $sql_search = "(1)";
    $sql_search .= " and wr_is_comment = '0' and left(wr_datetime, 10) = '{$date}' ";
...
// 한페이지에 표시 2020-07-06
$page_rows = $total_count > $page_rows ? $total_count : $page_rows;
...
if (!$is_search_bbs) {
    ...
    for ($k=0; $k<$board_notice_count; $k++) {
        ...
        $row = sql_fetch(" select * from {$write_table} where wr_id = '{$arr_notice[$k]}' and left(wr_datetime, 10) = '{$date}' ");
...
if ($is_search_bbs) {
    ...
    $sql = " select * from {$write_table} where wr_is_comment = 0 and {$sql_search} ";
    if(!empty($notice_array))

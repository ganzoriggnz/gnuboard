...
function na_sql_sort($type, $sort) {
	...
	if($type == 'coupon') {
		if(IS_NA_BBS) {
			switch($sort) { 
				case 'asc'			: $orderby = 'a.co_no'; break;
				case 'date'			: $orderby = 'a.co_created_datetime desc'; break;
				default				: $orderby = 'a.co_no desc'; break;
			}
		} else {
			switch($sort) { 
				case 'asc'			: $orderby = 'a.co_no'; break;
...
function na_board_rows_coupon($wset) {
	...
	if(!$bo_table || count($board_cnt) > 1 || $wset['bo_except']) {
		...
		// 공통쿼리
		$now = G5_TIME_YMDHIS;
		$sql_common = " from {$g5['coupon_table']} a, {$g5['board_table']} b where a.bo_table = b.bo_table and a.co_begin_datetime <= '{$now}' and b.bo_use_search = 1 $sql_plus $sql_minus $sql_term $sql_mb $sql_main $sql_where";
		if($page > 1) {
			$total = sql_fetch("select count(*) as cnt $sql_common ", false);
			$total_count = $total['cnt'];
			$total_page  = ceil($total_count / $rows);  // 전체 페이지 계산

...
function na_post_rows($wset, $subcat = '', $search = '')
{
	...
	if ($search == '') {
		for ($i = 0; $res = sql_fetch_array($result); $i++) {
			...
			if ($wset == '') {
				$result1 = sql_query("select *, exists (select 1 from g5_coupon c where c.mb_id = b.mb_id and c.co_end_datetime > now() and (c.co_free_num > '0' or c.co_sale_num > '0') and (c.co_free_num - c.co_sent_fnum != 0 and c.co_sale_num - c.co_sent_snum != 0)) has_coupon from  {$hwrite_table} a, {$g5['member_table']} b where a.wr_is_comment = 0 and a.mb_id = b.mb_id", false);
			} else if ($subcat == '') {
				$result1 = sql_query("select *, exists (select 1 from g5_coupon c where c.mb_id = b.mb_id and c.co_end_datetime > now() and (c.co_free_num > '0' or c.co_sale_num > '0') and (c.co_free_num - c.co_sent_fnum != 0 and c.co_sale_num - c.co_sent_snum != 0)) has_coupon from  {$hwrite_table} a, {$g5['member_table']} b where a.wr_is_comment = 0 and a.mb_id = b.mb_id and b.mb_2 like '%{$wset}%'", false);
			} else {
				$result1 = sql_query("select *, exists (select 1 from g5_coupon c where c.mb_id = b.mb_id and c.co_end_datetime > now() and (c.co_free_num > '0' or c.co_sale_num > '0') and (c.co_free_num - c.co_sent_fnum != 0 and c.co_sale_num - c.co_sent_snum != 0)) has_coupon from  {$hwrite_table} a, {$g5['member_table']} b where a.wr_is_comment = 0 and a.mb_id = b.mb_id and b.mb_2 like '%{$wset}%' and a.ca_name = '{$subcat}'", false);
			}
		...
		for ($i = 0; $res = sql_fetch_array($result); $i++) {
			...
			$bo_table = $res['bo_table'];
			$hwrite_table = $g5['write_prefix'] . $bo_table;
			$result1 = sql_query("select *, exists (select 1 from g5_coupon c where c.mb_id = b.mb_id and c.co_end_datetime > now() and (c.co_free_num > '0' or c.co_sale_num > '0') and (c.co_free_num - c.co_sent_fnum != 0 and c.co_sale_num - c.co_sent_snum != 0)) has_coupon from  {$hwrite_table} a, {$g5['member_table']} b where a.mb_id = b.mb_id and a.wr_is_comment = 0 and (b.mb_2 like '%{$search}%' or a.ca_name like '%{$search}%' or b.mb_name like '%{$search}%')", false);

...
// 표지 체크
$is_preface = ($write['wr_reply']) ? false : true;

// 표지면 통과
if($is_preface) {
	$preface_subject = get_text($view['wr_subject']);
} else {
	// 표지 불러오기
	$wr_preface = sql_fetch(" select wr_id, wr_subject, wr_comment, wr_good, wr_nogood, wr_1 from $write_table where wr_reply = '' and wr_num = '{$view['wr_num']}' ");

	// 서문 정보로 변경
	$preface_subject = get_text($wr_preface['wr_subject']);
	$category_name = $preface_subject; 
	$view['wr_comment'] = $write['wr_comment'] = $wr_preface['wr_comment'];
	$view['wr_good'] = $wr_preface['wr_good'];
	$view['wr_nogood'] = $wr_preface['wr_nogood'];

	// 글아이디 변경
	$wr_id = $wr_preface['wr_id'];
}
...
// 챕터 쓰기버튼 - 글쓴이만 가능(관리자도 안됨)
$reply_href = '';
if ($view['mb_id'] && $view['mb_id'] === $member['mb_id']) {
...
$chapter = array();
$i = 0;
$chresult = sql_query(" select wr_id, wr_subject from $write_table where wr_is_comment = '0' and wr_reply <> '' and wr_num = '{$view['wr_num']}' order by wr_2, wr_id ");
while ($chrow = sql_fetch_array($chresult)) {
	$chapter[$i] = $chrow;
	$i++;
}

$chapter_cnt = count($chapter);

// 현재 번호
$chapter_no = 1;
for($i=0; $i < $chapter_cnt; $i++) {
	if($chapter_id == $chapter[$i]['wr_id']) {
		$chapter_no = $i + 2;
		break;
	}
}

$chapter_num = sprintf("%02d", $chapter_no);

// 이전 챕터
$prev_chapter_href = '';
if($chapter_no > 1 && $chapter_no <= $chapter_cnt + 1 ) {
	if($chapter_no == "2") {
		$prev_wr = $wr_id;
	} else {
		$pcn = $i - 1;
		$prev_wr = $chapter[$pcn]['wr_id'];
	}
	$prev_chapter_href = get_pretty_url($bo_table, $prev_wr, $qstr);
} 

// 다음 챕터
$next_chapter_href = '';
if($chapter_no > 0 && $chapter_no < $chapter_cnt + 1 ) {
	$ncn = $chapter_no - 1;
	$next_wr = $chapter[$ncn]['wr_id'];
	$next_chapter_href = get_pretty_url($bo_table, $next_wr, $qstr);
} 
...
<article id="bo_v" class="mb-4">
			...
			<ul class="d-flex align-items-center">
				...
				<li id="bo_v_btn" class="flex-grow-1 text-right">
					...
					<div class="btn-group" role="group">
						...
						<?php if ($reply_href) { ?>
							<a href="<?php echo $reply_href ?>" class="btn btn_b01 nofocus py-1" title="챕터등록" role="button">
								<i class="fa fa-reply fa-md" aria-hidden="true"></i>
								<span class="sr-only">답변</span>
							</a>
						<?php } ?>
						<?php if ($write_href) { ?>
							<a href="<?php echo $write_href ?>" class="btn btn_b01 nofocus py-1" title="표지등록" role="button">
    ...
    <section id="bo_v_data">
		...
		<?php if($chapter_cnt) { // 챕터가 있으면 출력 ?>
			<form role="form" class="mt-3 px-3 px-sm-0 mb-0">
			<select name="chapter" class="custom-select" size="4" onchange="location=this.value;">
			<optgroup label="챕터 목록">
				<option value="<?php echo get_pretty_url($bo_table, $wr_id, $qstr) ?>"<?php echo ($is_preface) ? ' selected' : '';?>>
					01. <?php echo get_text($preface_subject);?>
				</option>
				<?php for($i=0; $i < $chapter_cnt; $i++) { ?>
					<option value="<?php echo get_pretty_url($bo_table, $chapter[$i]['wr_id'], $qstr) ?>"<?php echo ($chapter_id == $chapter[$i]['wr_id']) ? ' selected' : '';?>>
						<?php echo sprintf("%02d", $i + 2);?>. <?php echo get_text($chapter[$i]['wr_subject']);?>
					</option>
				<?php } ?>
			</optgroup>
			</select>

...
<section id="bo_w">
    ...
    <?php
	if($write['wr_good'] >= 100000)  echo $hit = "checked"; else echo ""; 
    $option = '';
    //$option_hidden = '';
    if ($is_notice || $is_html || $is_secret || $is_mail) {
        $option = '';
        if ($is_notice) {
            $option .= "\n".'<input type="checkbox" id="notice" name="notice" value="1" '.$notice_checked.'>'."\n".'<label for="notice">추천상품</label>';
        }

        if ($is_html) {
           $option .= "\n".'<input type="checkbox" id="wr_good" name="wr_good" value="100000" '.$hit.'>'."\n".'<label for="notice">특가상품</label>';
        }
    ...
    <div class="tbl_frm01 tbl_wrp">
        ...
        <?php } ?>
		<?php if($w == "") { ?>
		<input type="hidden" name="wr_9" value="<?php echo rand(100, 200).date("YmdHms") ?>"> <?php } ?>
		<?php if($w == "u") { ?>
    ...
    <div class="btn_confirm">
        <input type="submit" value="상품 올리기" id="btn_submit" accesskey="s" class="btn_submit">

...
Class form_option {
	...
	function Checkbox($opt='', $s_name, $s_id, $s_class, $s_val){
		...
		for($i=0; $i < count($data2); $i++) {
			...
			$result .= "<li><input type='checkbox' value='{$dataB[$i]}' name='{$s_name}[]' id='{$s_id}_{$i}' $checked class='{$s_class} hand'/>";
			$result .= "<label for='{$s_name}_{$i}' class='hand'> {$dataA[$i]}</label></li>";
		}
		$result .="</ul>";
	...
	function Int($init, $name, $id, $class, $val, $start, $end, $plus, $txt='') {

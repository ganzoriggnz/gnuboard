						...
						<div class="clearfix font-weight-normal bg-light border-top text-muted f-sm px-3 py-2<?php echo $by_writer ?>">
							<ul class="d-flex align-items-center">
								...
								<li class="flex-grow-1 text-right">
									...
									<time datetime="<?php if($list[$i]['wr_last'] != '') {echo date('Y-m-d\TH:i:s+09:00', strtotime($list[$i]['wr_last']));} else {echo date('Y-m-d\TH:i:s+09:00', strtotime($list[$i]['wr_datetime']));} ?>"><?php if($list[$i]['wr_last'] != '') { echo na_date($list[$i]['wr_last'], 'orangered', 'H:i', 'm.d H:i', 'Y.m.d H:i');} else {echo na_date($list[$i]['wr_datetime'], 'orangered', 'H:i', 'm.d H:i', 'Y.m.d H:i');} ?></time>
								</li>
							</ul>
						</div>
					</header>

var imageUrl='../skin/board/basic/img/color.png';
function iColorShow(id,id2)
{
	var eICP=jQuery("#"+id2).position();
	jQuery("#iColorPicker").css({'top':(event.pageY+10)+"px",'left':(event.pageX-250)+"px",'position':'absolute','border-radius':'5px','box-shadow':'5px 5px 10px #778899'}).fadeIn("fast");
	...
	{
		jQuery("input.iColorPicker").each(function(i){if(i==0)
		{
		...
		jQuery(document.createElement("div")).attr("id","iColorPickerBg").click(function(){jQuery("#iColorPickerBg").hide();jQuery("#iColorPicker").fadeOut()}).appendTo("body");
	...
	jQuery(this).css("backgroundColor",jQuery(this).val()).after('<a href="javascript:void(null)" id="icp_'+this.id+'" onclick="iColorShow(\''+this.id+'\',\'icp_'+this.id+'\')"><img src="'+imageUrl+'" style="border:0;margin:0 0 0 3px" align="absmiddle" >&nbsp;<span><font color="red">제</font><font color="blue">목</font><font color="green">컬</font><font color="brown">러</font></span></a>')})};
	jQuery(function(){iColorPicker()
})
/* 
$(function(){
	$("#iColorPicker").draggable();
}); */
